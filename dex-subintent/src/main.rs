...
async fn main() {
    ...
    let client = AnthicClient::new(network.clone(), trade_api_url.to_string(), cli.api_key);
    ...
    let address_info = client
        .load_account_address_info(user_address)
        .await
        .unwrap();
    ...
    let buy = TokenAmount {
        ...
        amount: Decimal::from_str(&cli.buy_amount).unwrap(),
    };
    let sell = TokenAmount {
        symbol: cli.sell_symbol,
        amount: Decimal::from_str(&cli.sell_amount).unwrap(),
    };

    // Create the manifest for the order
    let manifest =
        create_order_manifest(&anthic_config, user_address, &address_info, sell, buy).unwrap();
...
fn create_order_manifest(
    ...
    // in the transaction
    let solver_fee_amount = anthic_config
        .settlement_fee_per_resource
        .get(&sell.symbol)
        .unwrap()
        .clone();
    let anthic_fee_percent = anthic_config
        .anthic_fee_per_level
        .get(address_info.level as usize)
        .unwrap()
        .taker_fee
        .clone();
